// mock_view.js
// ゲームの仕様を満たすプログラムを記述するための仮の実行環境
// index.htmlのscriptに指定して検証する
// 検証はstart-buttonを押して始まり、target-decide-buttonを押して終了する
/*
...
let baseLatitude = 34.485746522;
let baseLongitude = 135.337352145;
...
// ゲームループ（非同期で動作）
function gameLoop() {
    if (!isPlaying) return; // ゲーム終了ならループしない
    ctx.clearRect(0, 0, canvas.width, canvas.height); // 画面をクリア
    drawTarget(400, 50);

    
    drawMarker(markerX, markerY); // 現在の位置を描画
    markerY --;
